...
const proxyFileIndex = process.argv.indexOf('--proxy');
const proxyFile = proxyFileIndex !== -1 ? (process.argv[proxyFileIndex + 1] || 'proxy.txt') : undefined;
...
let proxies = [];
let proxyIndex = 0;
let proxyRotationEnabled = false;
...
function loadProxiesFromFile(filePath) {
    try {
        ...
        proxyRotationEnabled = proxies.length > 0;
...
function getNextProxy() {
    ...
    proxyIndex = (proxyIndex + 1) % proxies.length; // Rotate back to start
...
function goProxy() {
    ...
    const netSocket = net.connect({
        ...
        // Send CONNECT request to proxy with auth if provided
        ...
        if (proxy.auth) {
            connectRequest += `Proxy-Authorization: Basic ${proxy.auth.encoded}\r\n`;
        }
        ...
        const onProxyData = (data) => {
            ...
            if (proxyResponse.includes('200 Connection established') || 
                proxyResponse.includes('200 OK') ||
                proxyResponse.match(/HTTP\/\d\.\d 200/)) {
                
                netSocket.removeListener('data', onProxyData);
                
                // FROM THIS POINT ON, EVERYTHING IS IDENTICAL TO THE ORIGINAL go() FUNCTION
            ...
            } else if (proxyResponse.length > 4096) {
                // Proxy didn't respond with 200, close connection
                netSocket.destroy();
            }
        };
        
        netSocket.on('data', onProxyData);

    }).once('error', () => { }).once('close', () => {
        if (tlsSocket) {
            tlsSocket.end(() => { tlsSocket.destroy(); goProxy(); });
        }
    });
    ...
    function cleanup(error) {
        if (error) {
            setTimeout(goProxy, getRandomInt(50, 200));
        }
        if (netSocket) {
            netSocket.destroy();
        }
...
if (cluster.isMaster) {
    ...
    // Load proxies if proxy file is specified
    if (proxyFile) {
        loadProxiesFromFile(proxyFile);
    }
    
    const useProxy = proxyRotationEnabled && proxies.length > 0;
