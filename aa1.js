...
async function bypassCloudflareOnce(attemptNum = 1) {
    ...
    try {
        ...
        // More aggressive content clearing
        await page.evaluate(() => {
...
async function bypassCloudflareParallel(totalCount) {
    console.log("\x1b[35mCLOUDFLARE BYPASS - SEQUENTIAL MODE (for lower memory)\x1b[0m");
    ...
    while (results.length < totalCount) {
        ...
        console.log(`\n\x1b[33mStarting batch (${currentBatchSize} session)...\x1b[0m`);
    ...
    process.on('message', (msg) => {
        if (msg.type === 'bypassData') {
            ...
            setInterval(() => {
                ...
                global.successRequests = 0;
                global.failedRequests = 0;
                if (global.gc) global.gc();  // GC in workers too
            }, 1000);
        }
    });
